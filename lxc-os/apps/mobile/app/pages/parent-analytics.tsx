...
export default function ParentAnalyticsPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            // Reusing the teacher's student-analytics endpoint as it should work if authorized
            const res = await api.get<AnalyticsData>(`/api/v1/dashboard/teacher/student-analytics?id=${activeStudentId}`);
            setData(res as any);
        } catch {
        } finally {
            setLoading(false);
    ...
    return (
        <View style={styles.container}>
            <PageHeader title="Progress Report" subtitle={`Performance analysis for ${data.student.user.name}`} />
            ...
            >
                ...
                <View style={styles.summaryRow}>
                    ...
                    <Animated.View entering={FadeInDown.delay(200)} style={styles.statCard}>
                        ...
                        <Text style={styles.statSub}>Across {data.examResults.length} assessments</Text>
                    </Animated.View>
                </View>
