...
function getDeploymentDomain() {
  // Check LearnXChain deployment environment variables first
  if (process.env.LearnXChain_INTERNAL_APP_DOMAIN) {
    return stripProtocol(process.env.LearnXChain_INTERNAL_APP_DOMAIN);
  }

  if (process.env.LearnXChain_DEV_DOMAIN) {
    return stripProtocol(process.env.LearnXChain_DEV_DOMAIN);
  }
  ...
  console.error(
    "ERROR: No deployment domain found. Set LearnXChain_INTERNAL_APP_DOMAIN, LearnXChain_DEV_DOMAIN, or EXPO_PUBLIC_DOMAIN",
  );
  process.exit(1);
}
