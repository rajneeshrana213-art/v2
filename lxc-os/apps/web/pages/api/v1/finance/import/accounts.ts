...
import fs from "fs";
import { FinanceImportService, AccountImportRow } from "@/lib/services/finance/FinanceImportService";
import { prisma } from "@/lib/prisma"; // Usually you verify auth here
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await FinanceImportService.importAccounts(sid, aid, records as AccountImportRow[], uid);

    return res.status(200).json(result);
  } catch (error: any) {
    console.error("Account Import API Error:", error);
