...
import fs from "fs";
import { FinanceImportService, OpeningBalanceImportRow } from "@/lib/services/finance/FinanceImportService";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await FinanceImportService.importOpeningBalances(sid, aid, records as OpeningBalanceImportRow[], uid);

    return res.status(200).json(result);
  } catch (error: any) {
    console.error("Opening Balances Import API Error:", error);
