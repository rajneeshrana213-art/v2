...
import { StaffBulkUploadFactory } from "@/lib/services/common/StaffBulkUploadFactory";
...
export default async function handler(
  ...
  try {
    ...
    const validatedData = bulkStaffUploadSchema.parse(enrichedData);

    // Phase 1 & 2: Pre-fetch and Validation Wall
    const metadata = await BulkUploadFactory.prefetchMetadata(user.schoolId);
    const { validRows, failedRows: validationErrors } = StaffBulkUploadFactory.validateRows(enrichedData, metadata);
    ...
    (async () => {
      try {
        ...
        await BulkUploadFactory.processChunksParallel(chunks, 5, async (chunk) => {
          const { results, errors } = await StaffBulkUploadFactory.processChunk(
            chunk as any, 
            user.schoolId, 
            metadata,
            async (success) => {
        ...
        const results = finalResults.map(res => ({
          ...res,
          staffName: res.user?.name || res.staffName,
          staffEmail: res.user?.email || res.staffEmail,
          staffUserName: res.credentials?.userName,
          staffRole: res.user?.role
        }));
        ...
        if (results.length > 0) {
          ...
          const successData = results.map((result, idx) => ({
            ...
            "Staff Name": result.staffName || "N/A",
            "Staff Email": result.staffEmail || "N/A",
            Username: result.staffUserName || "N/A",
            Password: result.credentials?.password || "N/A",
            Role:
              roleDisplayNames[result.staffRole] || result.staffRole || "N/A",
          }));
        ...
        if (errors.length > 0) {
          const errorData = errors.map((error) => ({
            ...
            "Staff Name": error.staffName,
            "Staff Email": error.staffEmail,
            Username: error.staffUserName,
            Role: error.staffRole,
        ...
        try {
          ...
          if (successFileBase64) {
            emailAttachments.push({
              filename: `staff_credentials_${istDateKey}.xlsx`,
          ...
          if (errorFileBase64) {
            emailAttachments.push({
              filename: `staff_upload_errors_${istDateKey}.xlsx`,
        ...
        await BulkUploadJobService.completeJob(jobId, {
          ...
          successFile: successFileBase64
            ? {
                filename: `staff_credentials_${getISTDateString()}.xlsx`,
          ...
          errorFile: errorFileBase64
            ? {
                filename: `staff_upload_errors_${getISTDateString()}.xlsx`,
    ...
    if (error.name === "ZodError") {
      ...
      const errorData = Array.from(errorsByRecord.entries()).map(
        ([index, errors]) => {
          ...
          return {
            "Row Number": index + 2,
            "Staff Name": record.name || "N/A",
            "Staff Email": record.email || "N/A",
    ...
    if (!res.headersSent) {
      return res
        .status(500)
        .json({ error: error.message || "Failed to bulk upload staff" });
    }
    res.end();
  }
}
