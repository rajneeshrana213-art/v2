...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const { classId, date, viewRange } = req.query;
        if (!classId || typeof classId !== "string") return res.status(400).json({ error: "Class ID required" });
        ...
        const data = await AdminAttendanceService.getClassAttendance(
            user.schoolId!, 
            classId, 
            referenceDate,
            startDate,
