...
async function handleGet(req: any, res: NextApiResponse, id: string) {
  try {
    const expense = await prisma.internalExpense.findUnique({
      where: { id },
      include: { category: true },
    });

    if (!expense) {
      return res.status(444).json({ message: "Expense record not found" });
    }
...
async function handlePut(req: any, res: NextApiResponse, id: string) {
  try {
    ...
    const existing = await prisma.internalExpense.findUnique({ where: { id } });
