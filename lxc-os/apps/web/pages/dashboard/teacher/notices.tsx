...
export default function TeacherNoticesPage() {
    ...
    useEffect(() => {
        const fetchNotices = async () => {
            try {
                const res = await client.get("/v1/dashboard/teacher/notices");
                setNotices(res.data);
            } catch (error) {
                ...
                <div className="space-y-6 pb-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Official announcements from school administration.</p>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-4">
                            {notices.length > 0 ? notices.map((notice, idx) => (
                                <div key={idx} className="group relative overflow-hidden rounded-3xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:border-indigo-500 dark:border-white/5 dark:bg-gray-900">
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {idx === 0 && (
                                                        <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-indigo-500 text-white animate-pulse">
                                                            New
                                                        </span>
                                                    )}
                                                </div>
                                    ...
                                    {notice.attachment && (
                                        <div className="mt-6 pt-4 border-t border-gray-50 dark:border-white/5">
                                            <a href={notice.attachment} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-bold text-indigo-600 hover:text-indigo-500">
                                ...
                                <div className="py-20 text-center bg-white dark:bg-gray-900 rounded-3xl border border-dashed border-gray-200 dark:border-white/10">
                                    ...
                                    <p className="text-gray-500">No active notices for teachers at the moment.</p>
                                </div>
                            )}
                        </div>
                    )}
