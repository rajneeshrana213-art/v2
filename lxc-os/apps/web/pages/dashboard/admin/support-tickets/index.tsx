...
import { Search, Filter, MoreVertical, Eye, CheckCircle, AlertCircle, Clock, XCircle, RefreshCw, Inbox, Paperclip, Download, File, Plus, Phone, Mail } from 'lucide-react';
...
export default function SupportTicketsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [statsRes, ticketsRes] = await Promise.all([
                getStats('/v1/dashboard/admin/tickets/stats', { autoToast: false }).catch(() => null),
                get(`/v1/dashboard/admin/tickets?page=${page}&search=${search}&status=${statusFilter}`)
            ]);
    ...
    const handleCreateSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await post('/v1/dashboard/admin/tickets', formData, {
            ...
            <div className="space-y-6 w-full px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 font-medium">Get help with your school dashboard</p>
                    </div>
                ...
                <div className="rounded-3xl shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-200 bg-white dark:border-white/10 dark:bg-gray-900 overflow-hidden">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-24">
                            ...
                            <p className="mt-2 text-gray-500 dark:text-gray-400 font-medium">Create a ticket if you need help with anything</p>
                        </div>
                    ) : (
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                    {tickets.map((ticket) => (
                                        <tr key={ticket.id} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors group">
                                            <td className="whitespace-nowrap px-8 py-6 font-black text-indigo-600 dark:text-indigo-400">
                                                #{ticket.ticketNumber}
                                            </td>
                                            ...
                                            <td className="whitespace-nowrap px-8 py-6">
                                                {ticket.employee ? (
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-9 w-9 rounded-full bg-indigo-100 flex items-center justify-center font-black text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400 overflow-hidden border border-indigo-100 dark:border-indigo-900/50">
                                                            {ticket.employee.user.profilePic ? (
                                                                <img src={ticket.employee.user.profilePic} alt="" className="h-full w-full object-cover" />
                                                            ) : ticket.employee.user.name.charAt(0)}
                                                        </div>
                                                        <div>
                                                            <p className="text-sm font-black text-gray-900 dark:text-white leading-tight">{ticket.employee.user.name}</p>
                                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">{ticket.employee.employeeType}</p>
                                                        </div>
                                                    </div>
                                                ) : (
                                ...
                                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-3xl bg-white p-8 shadow-2xl transition-all dark:bg-gray-900 border border-gray-200 dark:border-white/10">
                                    <div className="flex items-center justify-between mb-8">
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mt-1">Describe your issue and we'll help you out</p>
                                        </div>
                                                ...
                                                <div className="relative group">
                                                    ...
                                                    <label htmlFor="ticket-files" className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 rounded-3xl cursor-pointer bg-gray-50 hover:bg-gray-100 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 transition-all">
                                                        <div className="text-center">
                                                            ...
                                                            <p className="text-sm font-bold text-gray-700 dark:text-gray-300">Click to upload files</p>
                                                            <p className="text-[10px] font-black uppercase text-gray-400 mt-1">Images or PDFs (Max 5 files)</p>
                                                        </div>
                                                    </label>
                                                </div>
                                                {files.length > 0 && (
                                                        ...
                                                        <div className="rounded-3xl border border-gray-100 bg-gray-50/50 p-6 dark:border-white/5 dark:bg-white/5">
                                                            {selectedTicketDetail.employee ? (
                                                                ...
                                                                <div className="text-center py-4">
                                                                    ...
                                                                    <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Waiting for Admin</p>
                                                                    <p className="text-[10px] font-medium text-gray-500 mt-1">An expert will be assigned shortly</p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
