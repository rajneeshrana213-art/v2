...
export function useAudioRecorder(options: UseAudioRecorderOptions = {}) {
  ...
  const transcribeAudio = useCallback(
    async (audioBlob: Blob) => {
      ...
      try {
        ...
        onError?.(
          error instanceof Error ? error.message : 'भाषण पहचान विफल, कृपया पुनः प्रयास करें',
        );
      } finally {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (asrProviderId === 'browser-native') {
          ...
          if (!window.SpeechRecognition && !window.webkitSpeechRecognition) {
            onError?.('आपका ब्राउज़र भाषण पहचान का समर्थन नहीं करता');
            return;
          }
          ...
          recognition.onerror = (event: { error: string }) => {
            ...
            let errorMessage = 'भाषण पहचान विफल';

            switch (event.error) {
              case 'no-speech':
                errorMessage = 'कोई भाषण इनपुट नहीं मिला';
                break;
              case 'audio-capture':
                errorMessage = 'माइक्रोफ़ोन तक पहुँच नहीं हो सका';
                break;
              case 'not-allowed':
                errorMessage = 'माइक्रोफ़ोन की अनुमति अस्वीकृत';
                break;
              case 'network':
                errorMessage = 'नेटवर्क त्रुटि';
                break;
              default:
                errorMessage = `भाषण पहचान त्रुटि: ${event.error}`;
            }
      ...
      onError?.('माइक्रोफ़ोन तक पहुँच नहीं हो सका, कृपया अनुमति सेटिंग जाँचें');
    }
