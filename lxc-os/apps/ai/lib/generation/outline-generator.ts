...
export async function generateSceneOutlinesFromRequirements(
  ...
  let availableImagesText =
    requirements.language === 'hi-IN' ? 'कोई छवि उपलब्ध नहीं' : 'No images available';
  ...
  const prompts = buildPrompt(PROMPT_IDS.REQUIREMENTS_TO_OUTLINES, {
    ...
    mediaGenerationPolicy,
    researchContext:
      options?.researchContext || (requirements.language === 'hi-IN' ? 'कुछ नहीं' : 'None'),
  ...
  try {
    ...
    callbacks?.onProgress?.({
      ...
      overallProgress: 50,
      stageProgress: 100,
      statusMessage: `${result.length} दृश्य रूपरेखाएँ तैयार हो गईं`,
      scenesGenerated: 0,
