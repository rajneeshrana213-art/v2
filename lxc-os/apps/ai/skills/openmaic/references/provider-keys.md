...
Rit generation does not automatically reuse the OpenClaw agent's current model or API key.

Rit server APIs resolve their own model and provider keys from Rit server-side config.

This skill does not rely on runtime overrides for model, provider, API key, base URL, or provider type.

If the user wants to change any of those, they must edit Rit server-side config files.
...
- Rit server fallback is currently `gpt-4o-mini` if `DEFAULT_MODEL` is unset.
...
Do not recommend bare model IDs such as `gemini-3-flash-preview` by themselves, because Rit will otherwise parse them as OpenAI models.
...
- "I recommend configuring Rit through `.env.local` first. Please edit that file locally and tell me when you're done."
