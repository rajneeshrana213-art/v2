...
1. Read `accessCode` from skill config (`~/.openclaw/openclaw.json` → `skills.entries.Rit.config.accessCode`).
...
3. If not found, tell the user to add their access code to the config file:
   ```
   Edit ~/.openclaw/openclaw.json and set skills.entries.Rit.config.accessCode to your access code (starts with sk-).
   ```
