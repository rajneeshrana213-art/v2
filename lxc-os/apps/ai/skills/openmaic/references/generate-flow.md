...
- Rit is healthy at the selected `url`
- Provider keys are configured

> **Hosted mode**: If using hosted Rit (open.maic.chat), all
...
- Do not try to recover from auth, provider, model, or base URL errors by changing request parameters. Tell the user to fix Rit server-side config and retry only after they confirm.
