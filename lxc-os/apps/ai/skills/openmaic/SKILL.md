...
- Do not assume the OpenClaw agent's own model or API key will be reused by Rit.
- Rit classroom generation uses Rit server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Rit server-side config files may control provider selection and defaults.
...
If no `accessCode` in config, ask the user how they want to use Rit:

1. **Use hosted Rit** (recommended for quick start) — Requires an access code from open.maic.chat. No local setup needed.
...
Use this when the user has not installed Rit yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Rit

After the user has chosen a startup mode and configured keys, start Rit using the chosen method, then verify the service with `GET {url}/api/health`.
