...
export function MediaPopover({ onSettingsOpen }: MediaPopoverProps) {
    ...
    for (const p of Object.values(TTS_PROVIDERS)) {
      ...
      if (p.id === 'browser-native-tts' && browserVoices.length > 0) {
        ...
        for (const v of browserVoices) {
          ...
          const isHindi = langLower.startsWith('hi');
          const isEnglish = langLower.startsWith('en');
          const isIndianRegional = langLower.endsWith('-in');
          if (!isHindi && !isEnglish && !isIndianRegional) {
            continue; // Filter out Chinese and other foreign languages
          }
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      let testText = t('settings.ttsTestTextDefault');
      if (ttsProviderId === 'browser-native-tts') {
        const selectedVoice = browserVoices.find((v) => v.voiceURI === ttsVoice);
        if (selectedVoice) {
          const langLower = selectedVoice.lang.toLowerCase();
          const langCode = langLower.split('-')[0];
          if (langCode === 'hi') {
            testText = 'नमस्ते, यह एक परीक्षण वाक् है।';
          } else if (langCode === 'bn') {
            testText = 'নমস্কার, এটি একটি পরীক্ষা।';
          } else if (langCode === 'te') {
            testText = 'నమస్కారం, ఇది ఒక పరీక్షా ప్రసంగం.';
          } else if (langCode === 'ta') {
            testText = 'வணக்கம், இது ஒரு சோதனை பேச்சு.';
          } else if (langCode === 'mr') {
            testText = 'नमस्कार, हे एक चाचणी भाषण आहे.';
          } else if (langCode === 'gu') {
            testText = 'નમસ્તે, આ એક પરીક્ષણ ભાષણ છે.';
          } else if (langCode === 'kn') {
            testText = 'ನಮಸ್ಕಾರ, ಇದು ಪರೀಕ್ಷಾ ಭาಷಣವಾಗಿದೆ.';
          } else if (langCode === 'ml') {
            testText = 'നമസ്കാരം, ഇതൊരു പരീക്ഷണ പ്രസംഗമാണ്.';
          } else if (langCode === 'pa') {
            testText = 'ਸਤਿ ਸ੍ਰੀ ਅਕਾਲ, ਇਹ ਇੱਕ ਟੈਸਟ ਭਾਸ਼ਣ ਹੈ।';
          } else if (langCode === 'ur') {
            testText = 'ہیلو، یہ ایک ٹیسٹ اسپیچ ہے۔';
          } else if (langCode === 'en') {
            if (langLower.includes('in')) {
              testText = 'Hello, kaise ho aap? RIT Student Growth OS is ready.';
            } else {
              testText = 'Hello, this is a test speech from RIT student growth operating system.';
            }
          }
        }
      }
      ...
      Object.values(ASR_PROVIDERS)
        ...
        .map((p) => ({
          ...
          items: getASRSupportedLanguages(p.id).map((l) => ({
            id: l,
            name: t('settings.lang_' + l) === 'settings.lang_' + l ? l : t('settings.lang_' + l),
          })),
        })),
    [asrProvidersConfig, t],
  );
