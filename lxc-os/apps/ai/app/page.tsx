...
function HomePage() {
  ...
  const showWorkspace = searchParams.get('workspace') === 'true';
  const isDev = typeof window !== "undefined"
    ? (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1") && process.env.NODE_ENV !== "production" && process.env.APP_ENV !== "production"
    : (process.env.NODE_ENV === "development" && process.env.APP_ENV !== "production");
  ...
  if (authStatus === 'loading') {
    return (
      <div className="min-h-screen w-full bg-slate-50 dark:bg-[#050d17] flex flex-col items-center justify-center gap-4 transition-colors duration-300 font-sans">
        <div className="relative w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center bg-[#0057C8]/10 dark:bg-[#0057C8]/20 animate-pulse">
          <Zap className="w-10 h-10 text-[#0057C8] dark:text-[#1A9FFF]" />
        </div>
        <span className="text-2xl font-extrabold text-slate-900 dark:text-white animate-pulse" style={{ fontFamily: 'Syne, sans-serif' }}>
          RIT <span className="text-[#5CDD2B]">AI</span>
        </span>
      </div>
    );
  }

  // Always show Landing Page at root `/` unless specifically accessing the workspace mode
  if (authStatus === 'unauthenticated' || !showWorkspace) {
    return <LandingPage />;
  }

  return (
    <div className="min-h-[100dvh] w-full bg-gradient-to-b from-sky-50 to-blue-50/60 dark:from-[#0c1522] dark:to-[#0a1929] flex flex-col items-center p-4 pt-16 md:p-8 md:pt-16 overflow-x-hidden">
      ...
      >
        ...
        {/* LXC Growth OS Button */}
        <Link
          href="/lxc"
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-semibold text-[#1a6fd8] hover:bg-[#1a6fd8]/10 transition-all"
          title="LearnXChain Student Growth OS"
        >
          <Brain className="w-3.5 h-3.5" />
          <span className="hidden sm:block">LXC</span>
        </Link>
      ...
      >
        ...
        <motion.img
          ...
          className="h-16 md:h-20 mb-2 rounded-2xl"
        />
        ...
        <motion.div
          ...
          className="w-full"
        >
          <div className="w-full rounded-2xl border border-border/60 bg-white/80 dark:bg-[#0f1f35]/90 backdrop-blur-xl shadow-xl shadow-black/[0.03] dark:shadow-black/20 transition-shadow focus-within:shadow-2xl focus-within:shadow-blue-500/[0.08]">
            ...
            <textarea
              ...
              onFocus={() => {
                // Do nothing, handled by early return
              }}
