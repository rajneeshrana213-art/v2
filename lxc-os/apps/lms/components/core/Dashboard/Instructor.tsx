  ...
  useEffect(() => {
    const getCourseDataWithStats = async () => {
      ...
      try {
        ...
        const coursesResponse = await fetch("/api/course/instructor")
        const coursesResult = await coursesResponse.json()

        if (statsResult.success) {
          setInstructorData(statsResult.courses)
        }
      ...
      {loading ? (
        ...
        <div>
          ...
          <div className="rounded-md bg-richblack-800 p-6">
            ...
            <div className="my-4 flex flex-wrap lg:flex-nowrap items-start gap-6">
              {courses.slice(0, 3).map((course) => (
                <div key={course.id} className="w-full lg:w-1/3">
                  <img
